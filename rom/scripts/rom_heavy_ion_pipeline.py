#!/usr/bin/env python3
"""Generate a MASK NOR-ROM read-error cross-section from circuit Qcrit.
...
def run_read1(
    ...
    *,
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".cir", prefix="rom_joint_qcrit_", delete=False
    ) as handle:
        handle.write(source)
...
def find_qcrit(template: str, circuit_dir: Path, *, max_q_fc: float, tolerance_fc: float, **case) -> tuple[float, str]:
    if not run_read1(template, circuit_dir, q_fc=0.0, **case):
        return 0.0, "baseline_failed"
    if run_read1(template, circuit_dir, q_fc=max_q_fc, **case):
        return max_q_fc, "right_censored"
    lo, hi = 0.0, max_q_fc
    while hi - lo > tolerance_fc:
        mid = 0.5 * (lo + hi)
        if run_read1(template, circuit_dir, q_fc=mid, **case):
            lo = mid
        else:
            hi = mid
    return hi, "resolved"
...
def integrate_spenvis_let(
    ...
    for level_name in sorted({row["level"] for row in curve}):
        ...
        def integrate(values: np.ndarray) -> float:
            ...
            return float(np.trapz(dflux * interp, let_mg))
...
def make_parser() -> argparse.ArgumentParser:
    ...
    char.add_argument("--netlist", default="rom_hierarchical_senseamp.cir")
    ...
    char.add_argument("--seed", type=int, default=20260808)
    ...
    char.add_argument("--output", default="results/rom_joint_qcrit_samples.tsv")
    ...
    cross.add_argument("--qcrit-input", default="results/rom_joint_qcrit_samples.tsv")
    cross.add_argument("--seed", type=int, default=20260808)
    ...
    cross.add_argument("--heavy-ion-output", default="results/rom_heavy_ion_cross_section.tsv")
    cross.add_argument(
        "--spenvis-let",
    ...
    cross.add_argument("--rate-output", default="results/rom_spenvis_heavy_ion_rate.tsv")
